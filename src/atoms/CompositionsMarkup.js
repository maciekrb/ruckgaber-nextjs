import React from 'react'
import { makeStyles } from '@material-ui/core/styles'
import { red } from '@material-ui/core/colors'
import PictureAsPdfIcon from '@material-ui/icons/PictureAsPdf'
import Typography from '@material-ui/core/Typography'

const useStyles = makeStyles((theme) => ({
  root: {
    '& .section-ref': {
      marginBottom: '40px',
    },
    '& .c0': {
      paddingTop: '0pt',
      paddingBottom: '0pt',
      lineHeight: 1.7,
      orphans: 2,
      widows: 2,
      textAlign: 'left',
      marginRight: '-30.4pt',
      marginBlockStart: '0.8em',
      marginBlockEnd: '0.8em',
    },
    '& .MuiSvgIcon-root': {
      position: 'relative',
      top: '5px',
    },
    '& .c5': {
      color: '#000000',
      fontWeight: 700,
      textDecoration: 'none',
      verticalAlign: 'baseline',
      fontSize: '11pt',
    },
    '& .c9': {
      textDecoration: 'none',
      verticalAlign: 'baseline',
      fontSize: '11pt',
      fontStyle: 'normal',
    },
    '& .c2': {
      color: '#000000',
      textDecoration: 'none',
      verticalAlign: 'baseline',
      fontSize: '11pt',
      fontStyle: 'normal',
    },
    '& .c16': {
      color: '#000000',
      textDecoration: 'none',
      verticalAlign: 'baseline',
      fontStyle: 'normal',
    },
    '& .c32': {
      color: '#1155cc',
      textDecoration: 'underline',
    },
    '& .c26': {
      color: '#000000',
      textDecoration: 'none',
      fontSize: '16pt',
      fontStyle: 'normal',
    },
    '& .c18': {
      color: '#000000',
      textDecoration: 'none',
      fontSize: '11pt',
      fontStyle: 'normal',
    },
    '& .c6': {
      fontSize: '10pt',
      fontWeight: 400,
    },
    '& .c17': {
      textDecoration: 'none',
      verticalAlign: 'baseline',
      fontStyle: 'normal',
    },
    '& .c19': {
      color: '#000000',
      textDecoration: 'none',
      fontSize: '11pt',
    },
    '& .c27': {
      color: '#000000',
      textDecoration: 'none',
    },
    '& .c33': {
      textDecoration: 'none',
      fontSize: '11pt',
    },
    '& .c44': {
      listStylePosition: 'inside',
      textIndent: '0pt',
    },
    '& .c30': {
      fontWeight: 400,
    },
    '& .c1': {
      fontWeight: 400,
    },
    '& .c46': {
      color: 'inherit',
      textDecoration: 'inherit',
    },
    '& .c4': {
      verticalAlign: 'super',
      fontSize: '12pt',
    },
    '& .c35': {
      padding: 0,
      margin: 0,
    },
    '& .c8': {
      fontWeight: 700,
    },
    '& .c48': {
      fontWeight: 700,
    },
    '& .c28': {
      fontStyle: 'normal',
    },
    '& .c10': {
      color: '#ff0000',
    },
    '& .c13': {
      color: '#3366ff',
    },
    '& .c50': {
      fontSize: '4pt',
    },
    '& .c45': {
      color: '#9900ff',
    },
    '& .c23': {
      verticalAlign: 'baseline',
    },
    '& .c41': {
      fontSize: '15pt',
    },
    '& .c21': {
      height: '11pt',
    },
    '& .c22': {
      fontSize: '13pt',
    },
    '& .c36': {
      fontSize: '12pt',
    },
    '& .c11': {
      verticalAlign: 'super',
    },
    '& .c37': {
      fontSize: '11pt',
    },
    '& .c15': {
      fontSize: '9pt',
    },
    '& .c43': {},
    '& .c38': {
      fontSize: '16pt',
    },
    '& .c25': {
      fontSize: '8pt',
    },
    '& .c3': {
      fontStyle: 'italic',
    },
    '& .c14': {
      fontSize: '10pt',
    },
    '& .c60': {
      fontSize: '6pt',
    },
    '& .c34': {
      color: '#0000ff',
    },
    '& .c12': {
      fontSize: '14pt',
    },
    '& .c42': {
      fontSize: '7pt',
    },
    '& .c40': {
      fontSize: '18pt',
    },
  },
}))

const CompositionsMarkup = () => {
  const classes = useStyles()
  return (
    <div className={classes.root}>
      <a className="section-ref" id="section-A" />
      <Typography variant="h3" gutterBottom>
        A. Music for the Piano with an Orchestra{' '}
      </Typography>
      <p className="c0">
        <span className="c8 c3">A.1. Rondo de concert As-Dur,</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c3 c14">op. 17,</span>
        <span className="c1 c14">&nbsp;</span>
        <span className="c1 c11 c60">1, 2, 3</span>
        <span className="c1 c3">Piano et Orchestre, 1853, J. Ruckgaber, </span>
        <span className="c1 c15">ms</span>
        <span className="c1 c22 c3">.</span>
        <span className="c1 c11">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.1.%20Rondo%20de%20concert%20op.17%20%28366-368%29%20obraz%20497-506.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c2 c1">&nbsp; </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          A.2a. Partition des grandes Variationes sur un th&egrave;me polonais,
        </span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c15 c3">op. 18, </span>
        <span className="c1 c15">&nbsp;</span>
        <span className="c1 c11 c60">1, 2, 3</span>
        <span className="c1 c3">
          pour Piano avec L&rsquo;accompagnement de tout L&rsquo;orchestre,{' '}
        </span>
        <span className="c1 c15">ms</span>
        <span className="c1">.</span>
        <span className="c1 c11 c38">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.2.a.%20Partition%20des%20grandes%20Variationes%2C%20op.%2018%20%28330-359%29%20obraz%20388-409.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c2 c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          A.2b. Grandes Variations avec l&rsquo;Introduction et Finale,{' '}
        </span>
        <span className="c1 c3">op.18, </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c11 c60">2, 4</span>
        <span className="c1 c3">
          pour le piano aves tout orchestre, dediees a M-elle Jeanette Hofman
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.a. </span>
        <span className="c1 c3">Concerto pour Piano par Jean Ruckgaber </span>
        <span className="c1">, </span>
        <span className="c1 c11 c60">2, 3, 4</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c22">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.a.%20%28330-359%29obraz%20058%20tylko%20ok%C5%82adka.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c15">(cover only)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.b. </span>
        <span className="c1 c3">
          Concerto pour le Piano Forte, par Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">2, 3, 4</span>
        <span className="c1 c3">
          avec L&rsquo;accompagnement de grande Orchestre,
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Partition, </span>
        <span className="c1 c22">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.b.%20Concerto%20pour%20le%20Piano%20Forte%2C%20avec%20L%E2%80%99accompagnement%20de%20grande%20%E2%80%A8Orchestre%2C%20Partition%20%28366-368%29%20obr.382-428.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c15">&nbsp;(probably mixed pages)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.c.</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1">&nbsp; </span>
        <span className="c1 c3">
          Concerto pour le Piano, avec l&rsquo;accompagnement de grand
          Orchestre,
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c11 c60">2, 3, </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.c.%20Concerto%20pour%20le%20Piano%2C%20avec%20l%E2%80%99accompagnement%20de%20grand%20%E2%80%A8Orchestre%20%28330-359%29%20%28330-359%29%20obr.122-150%2C%20brak%20str.1%2C2%2C3%20re%CC%A8kopisu.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c1 c15">&nbsp;</span>
        <span className="c1 c15">
          (missing pages 1,2,3 of the manuscript, or is it &nbsp;A.5?)
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.d.</span>
        <span className="c1 c3">&nbsp;Concerto pour le Piano</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Viola par Jean Ruckgaber, </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c11 c60">2, 3, 4</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.d.%20Concerto%2C%20Viola%20%28330-359%29%20obr.190b-195a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.e.</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c3">Concerto pour le Piano</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Corno 2ndo, par Jean Ruckgaber, </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c11 c60">3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.c.%20Concerto%20pour%20le%20Piano%2C%20avec%20l%E2%80%99accompagnement%20de%20grand%20%E2%80%A8Orchestre%20(330-359)%20(330-359)%20obr.122-150%2C%20brak%20str.1%2C2%2C3%20r%C4%99kopisu.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.f.</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c3">Concerto, Tympanie, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.f.%20Concerto%2C%20Tympanie%20%28330-359%29%20obraz%20203-204a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.g.</span>
        <span className="c6 c3">&nbsp;</span>
        <span className="c1 c3">
          Concerto, Trombone Basso, par Jean Ruckgaber,{' '}
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.g.%20Concerto%2C%20Trombone%20Basso%20%28330-359%29%20obraz%20204b-205a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.h.</span>
        <span className="c6 c3">&nbsp;</span>
        <span className="c1 c3">Concerto, Fagotto 2ndo, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.h.%20Concerto%2C%20Fagotto%202ndo%20%28330-359%29%20obraz%20205b-207.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.i.</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c3">Concerto, Fagotto 1mo, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c15">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.i.%20Concerto%2C%20Fagotto%201mo%20%28330-359%29%20obraz%20208-211a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.j.</span>
        <span className="c6 c3">&nbsp;</span>
        <span className="c1 c3">Concerto, Trombe 2ndo, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.j.%20Concerto%2C%20Trombe%202ndo%20%28330-359%29%20obraz%20211b-212a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.3.k.</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c3">Concerto, Trombe 1mo, par Jean Ruckgaber, </span>
        <span className="c1 c11 c3">&nbsp;</span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c22">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.3.k.%20Concerto%2C%20Trombe%201mo%20%28330-359%29%20obraz%20212b.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          A.4. Premier concerto pour le Piano Forte, op.20,{' '}
        </span>
        <span className="c1 c11 c60">1, 2, 3, 4 </span>
        <span className="c1 c3">
          avec l&rsquo;accompagnement de grand Orchestre, 1872,{' '}
        </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.4.%20Premier%20Concerto%20op.20%20%28330-356%29%20obr.356-387.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c10 c15">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.a.</span>
        <span className="c1 c3">
          Violino principale du Grande Concert pour le Piano Forte,
        </span>
        <span className="c1 c11 c60">&nbsp;3, 4</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Dediee a son ami Charles Lipinsky, </span>
        <span className="c1 c15 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.a.%20Violino%20Principale%20du%20Grande%20Concert%20%28330-359%29%20obr.151-156.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.b</span>
        <span className="c1 c3">.</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Viola du Grande Concert, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3, 4</span>
        <span className="c1 c22">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.b.%20Viola%20du%20Grande%20Concert%20%28330-359%29%20obr.%20157-161.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.c. </span>
        <span className="c1 c3">
          Basso Violoncello Grande Concert par Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c22 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.c.%20Basso%20Violoncello%20Grande%20Concert%20%28330-359%29%20obr.%20195b-198a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c16 c1 c22">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.d.</span>
        <span className="c1 c3">&nbsp;Grand Concerto Violino II</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">pour J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.d.%20Grand%20Concerto%20Violino%20II%20%28330-359%29%20obraz%20178-182.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.e.</span>
        <span className="c1">&nbsp; </span>
        <span className="c1 c3">Grand Concerto Corno 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">par J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.e.%20Grand%20Concerto%20Corno%201mo%20%28330-359%29%20obr.%20167-170.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.f.</span>
        <span className="c1 c3">
          &nbsp;Gr. Concerto Violoncelle e Contra Basso pour Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.f.%20Gr.%20Concerto%20Violoncelle%20e%20Contra%20Basso%20%28330-359%29%20obr.%20171-177.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.g</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Grand Concerto Clarinetto 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">in C par J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.g.%20Grand%20Concerto%20Clarinetto%201mo%20%28330-359%29%20obraz%20216-219a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c22">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.h.</span>
        <span className="c1 c3">&nbsp;Grand Concerto Oboe 2do</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">par J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.h.%20Grand%20Concerto%20Oboe%202do%20%28330-359%29%20obraz%20219b-221a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c8 c38 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.i.</span>
        <span className="c1 c3">&nbsp;Grand Concerto Oboe 1mo</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">par J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.i.%20Grand%20Concerto%20Oboe%201mo%20%28330-359%29%20obraz%20221b-224.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.j.</span>
        <span className="c1 c3">
          &nbsp;Grand Concerto Trombe 1mo par J. Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.j.%20Grand%20Concerto%20Trombe%201mo%20(330-359)%20obraz%20213a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.k.</span>
        <span className="c1 c3">
          &nbsp;Grand Concerto Clarinetto 2do par J. Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.k.%20Grand%20Concerto%20Clarinetto%202do%20%28330-359%29%20obraz%20213b-215.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c1 c22">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.l.</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Gr. Concert Violino 2do</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">pour J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.l.%20Gr.%20Concert%20Violino%202do%20%28330-359%29%20obraz%20185-189.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.5.m.</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Grande Concerto, Viola</span>
        <span className="c1">&nbsp;pour J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.5.m.%20Grande%20Concerto%2C%20Viola%20%28330-359%29%20obraz%20198b-202.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          A.6. Einzugs Marsch zur Jungfrau von Orlean, op. 25,
        </span>
        <span className="c1 c11 c60">1, 2, 3, 4 </span>
        <span className="c2 c1">&nbsp;</span>
        <span className="c1 c3">
          eine romantische Trag&ouml;die v. Schiller, f&uuml;r Orchester und
          Piano,
        </span>
        <span className="c1 c15 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.6.%20Einzugs%20Marsch%20zur%20Jungfrau%20von%20Orlean%20op.25%28330-359%29%20obr.%20428-429.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.7.</span>
        <span className="c8">&nbsp;</span>
        <span className="c8 c3">Adagio de concert, op. 38, </span>
        <span className="c1 c11 c60">1,2, 3</span>
        <span className="c1 c22">&nbsp;</span>
        <span className="c1 c3">en mi-mineur, piano avec l&rsquo;orchestre</span>
        <span className="c1 c11">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.7.a. </span>
        <span className="c1 c11 c60">2</span>
        <span className="c1 c3 c12">&nbsp;</span>
        <span className="c19 c1 c23 c3">
          Adagio de Concert en Mi mineur avec l&rsquo; accompagnement de
          l&rsquo;orchestre pour Piano, op.38, &ldquo;onpent executer cette
          morceau pour piano seul en jouant seulement les petites notes&rdquo;
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.7.b. </span>
        <span className="c1 c11 c60">2,3</span>
        <span className="c1 c3 c12">&nbsp;</span>
        <span className="c1 c3">
          Adagio pour piano, tiree du premier concert avec l&rsquo;
          accompagnement de l&rsquo;orchestre, op.38, &ldquo;Zum Jahre 1872
          ganzliche Umarbeitung&rdquo;.{' '}
        </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.7.b.%20Adagio%20op.%2038%20%28366-368%29%20obr.343-364.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.7.c. </span>
        <span className="c1 c11 c60">2,3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">
          Adagio de Concert en mi mi miner, piano avec l&rsquo;orchestre
          &bdquo;onpent executer cette morceau pour piano seul en jouant seulement les petites notes” &rdquo;{' '}
        </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.7.c.%20Adagio%20de%20concert%2C%20op.38%20%28366-368%29obraz365-381.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.8. Rondo de concert, op. 40, </span>
        <span className="c1 c11 c60">2, 3</span>
        <span className="c2 c1">&nbsp;</span>
        <span className="c1 c3">pour le</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">
          Piano Forte avec L&rsquo;accompagnement de grande Orchestre,
        </span>
        <span className="c1 c11 c60">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.8.%20Rondo%20de%20concert%2C%20op.40%20%28F366-368%29%20obr.454-468%20cz.3.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.a. &nbsp;</span>
        <span className="c1 c3">Rondo tiree du 1mo Concert, en mi mineur </span>
        <span className="c1">,</span>
        <span className="c1 c11 c60">3, 4 </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c15">ms. </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.a.%20Rondo%20tiree%20du%201mo%20Concert%2C%20en%20mi%20mineur%20%28330-359%29%20obr.059-067.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.b. </span>
        <span className="c1 c3">Rondo de Concert op. 41, Corno 2</span>
        <span className="c1 c11 c60">00</span>
        <span className="c1 c3">
          &nbsp;E.G.Es.E Allegretto, par Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.b.%20Rondo%20de%20Concert%20op.41%2C%20Corno%202do%20%28330-359%29%20obraz%20074-075.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">&nbsp;A.9.c.</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Rondo de Concert Op. 41, Trombe 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, par Jean Ruckgaber, </span>
        <span className="c1 c11">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.c.%20Rondo%20de%20Concert%20op.41%2C%20Trombe%201mo%20%28330-359%29%20obraz%20077-078.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.d. </span>
        <span className="c1 c3">Rondo de Concert Op. 41, Trombe 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1 c3">, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.d.%20Rondo%20de%20Concert%20op.41%2C%20Trombe%202do%20%28330-359%29%20obraz%20079-080a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.e. </span>
        <span className="c1 c3">Rondo de Concert Op. 41, Clarinetto 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.e.%20Rondo%20de%20Concert%20op.41%2C%20Clarinetto%201mo%20%28330-359%29%20obraz%20080b-082a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.f. </span>
        <span className="c1 c3">Rondo de Concert Op. 41, Clarinetto 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1 c3">, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.f.%20Rondo%20de%20Concert%20op.41%2C%20Clarinetto%202do%20%28330-359%29%20obraz%20082b-083.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.g. </span>
        <span className="c1 c3">Rondo de Concert Op. 41, Fagotto 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.g.%20Rondo%20de%20Concert%20op.41%2C%20Fagotto%201mo%20%28330-359%29%20obraz%20084-086.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.h. </span>
        <span className="c1 c3">Rondo de Concert Op. 41,Tympani</span>
        <span className="c1 c3">,par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.h.%20Rondo%20de%20Concert%20%20op.41%2C%20Tympani%20%28330-359%29%20obraz%20087-088a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.i. </span>
        <span className="c1 c3">Rondo de Concert Op. 41, Fagotto 2</span>
        <span className="c1 c11 c60">do </span>
        <span className="c1 c3">, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.i.%20Rondo%20de%20Concert%20%20op.41%2C%20Fagotto%202do%20%28330-359%29%20obraz%20088b-090.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">&nbsp;A.9.j. </span>
        <span className="c1 c3">
          Rondo de Concert op. 41, Trombone Basso, par Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.j.%20Rondo%20de%20Concert%20op.41%2C%20Trombone%20Basso%20%28330-359%29%20obraz%20091-092a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">&nbsp;A.9.k. </span>
        <span className="c1 c3">
          Rondo de Concert op. 41, Basso et Violoncello, par Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.k.%20Rondo%20de%20Concert%20op.41%2C%20Basso%20et%20Violoncello%20%28330-359%29%20obraz%20092b-095.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">&nbsp;A.9.l. </span>
        <span className="c1 c3">Rondo de Concert op. 41, Violino 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, principale,</span>
        <span className="c8 c3">&nbsp;</span>
        <span className="c1 c3">par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.l.%20Rondo%20de%20Concert%20op.41%2C%20Violino%201mo%2C%20principale%20%28330-359%29%20obraz%20096-099.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">&nbsp;A.9.m. </span>
        <span className="c1 c3">Rondo de Concert op. 41, Violino 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, par Jean Ruckgaber </span>
        <span className="c1">,</span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c9 c1">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.m.%20Rondo%20de%20Concert%20op.41%2C%20Violino%201mo%20%28330-359%29%20obraz%20100-104.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span class="c16 c1 c15">(page 103 seems added)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">&nbsp;A.9.n.</span>
        <span className="c1 c3">&nbsp;Rondo de Concert op. 41, Violino 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1 c3">, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">&nbsp;3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.n.%20Rondo%20de%20Concert%20op.41%2C%20Violino%202do%20%28330-359%29%20obraz%20105-108.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">&nbsp;A.9.o. </span>
        <span className="c1 c3">
          Rondo de Concert op. 41, Viola, par Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c9 c1">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.o.%20Rondo%20de%20Concert%20op.41%2C%20Viola%20%28330-359%29%20obraz%20109-110a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c30 c15">(doubtful pictures 111 i 112)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.p. </span>
        <span className="c1 c3">Rondo de Concert op. 41, Flauto 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, par J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.p.%20Rondo%20de%20Concert%20op.41%2C%20Flauto%201mo%20%28330-359%29%20obraz%20113-115a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.r. </span>
        <span className="c1 c3">Rondo de Concert op. 41, Flautino 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, par J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.r.%20Rondo%20de%20Concert%20op.41%2C%20Flautino%201mo%20%28330-359%29%20obraz%20115b-116a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.s. </span>
        <span className="c1 c3">Rondo de Concert op. 41, Oboe 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, par J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.s.%20Rondo%20de%20Concert%20op.41%2C%20Oboe%201mo%20%28330-359%29%20obraz%20116b-118a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c2 c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.t. </span>
        <span className="c1 c3">Rondo de Concert op. 41, Oboe 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1 c3">, par J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.t.%20Rondo%20de%20Concert%20op.41%2C%20Oboe%202do%20%28330-359%29%20obraz%20118b-119.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.9.u. </span>
        <span className="c1 c3">Rondo de Concert op. 41, &nbsp;Corno 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, par J. Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.9.u.%20Rondo%20de%20Concert%20op.41%2C%20Corno%201mo%20%28330-359%29%20obraz%20120-121.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.10. Grande Ouverture de Concert, </span>
        <span className="c1 c3">Partition,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">par Johan Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.10.%20Grande%20Ouverture%20de%20Concert%2C%20Partition%20%20(330-359)%20obraz%20162-166.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c14">
          (the cover does not match the music, see A.12?)
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          A.11. Two Marches, op. 44, dedicated to the Lviv National Guard,{' '}
        </span>
        <span className="c1 c3">ca 1850 </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c11 c60">1, 2, 3, 4, 6 &nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.11.%20Dwa%20marsze%20Gwardyi%20Narodowej%20Lwowskiej%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">6</span>
        <span className="c1">,</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.11.a.%20(280-329)%20obraz%20279-285.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">3 </span>
        <span className="c1">,</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.11.b.%20(330-359)%20obraz%20279-284.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">3 </span>
        <span className="c1 c15">(the same cover, different music)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.a. </span>
        <span className="c1 c3">Grande Ouverture de Concert,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">pour grande Orchestra, Partition, </span>
        <span className="c1 c11 c60">2, 3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.a.1.%20(280-329)%20obraz%20161-227.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">,</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.a.2.%20(280-329)%20obraz%20228-231.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.b.</span>
        <span className="c1 c3">&nbsp;Grande Ouverture de Concert</span>
        <span className="c1">, Op. 63 </span>
        <span className="c1 c3">&ndash;Partition</span>
        <span className="c1">, </span>
        <span className="c1 c3">pour grande Orchestre </span>
        <span className="c1 c11 c60">2, 3, 4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.b.%20Grande%20Ouverture%20de%20Concert%20op.63%2C%20Partition%20(330-359)%20obraz%20441-569.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.c.1.</span>
        <span className="c8">&nbsp;</span>
        <span className="c1 c3">
          Grand Ouverture de Concert, Violino 1mo Directore, par Jean Ruckgaber
        </span>
        <span className="c1 c11 c60">3, 4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.c.1.%20Grande%20Ouverture%20de%20Concert%2C%20Violino%201mo%20Directore%20(280-329)%20obraz%20074-095.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.c.2. </span>
        <span className="c1 c3">Grande Ouverture de Concert,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">
          Violino 1mo Directore, par Jean Ruckgaber, 1863
        </span>
        <span className="c1 c11 c60">3, 4 </span>
        <span className="c1 c10 c15">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.c.2.%20Grande%20Ouverture%20de%20Concert%2C%20Violino%201mo%20Directore%20(330-359)%20obraz%20570-581.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.c.3. </span>
        <span className="c1 c3">Grand Ouverture de concert, Violino 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1">, </span>
        <span className="c1 c3">Andante con moto,</span>
        <span className="c8">&nbsp;</span>
        <span className="c1 c3">par Jean Ruckgaber </span>
        <span className="c1">,</span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10 c15">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.c.3.%20Grande%20Ouverture%20de%20Concert%2C%20Violino%201mo%20(280-329)%20obraz%20110b-119a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.d.1. </span>
        <span className="c1 c3">Grand Ouverture de concert, Violino 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1 c3">, Andante con moto, par Jean Ruckgaber, </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c11 c60">2, 3</span>
        <span className="c19 c1 c23 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.d.1.a.%20Grande%20Ouverture%20de%20Concert%2C%20Violino%202do%20(280-329)%20obraz%20096-101.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">, </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.d.2.%20Grande%20Ouverture%20de%20Concert%2C%20Violino%202do(280-329)%20obraz%20119b-128a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.d.2. </span>
        <span className="c1 c3">Grand Ouverture de concert, Violino 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1">, </span>
        <span className="c1 c3">Andante con moto,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c10 c15">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.d.2.%20Grande%20Ouverture%20de%20Concert%2C%20Violino%202do%28280-329%29%20obraz%20119b-128a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c1 c25">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.d.3. </span>
        <span className="c1 c3">Grande Ouverture, Violino 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1 c3">, Andante con moto, par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.d.3.%20Grande%20Ouverture%20de%20Concert%2C%20Violino%202do%20(280-329)%20obraz%20128b-133a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.e. </span>
        <span className="c1 c3">Grand Ouverture de concert, Viola</span>
        <span className="c1">, </span>
        <span className="c1 c3">Andante con moto,</span>
        <span className="c8">&nbsp;</span>
        <span className="c1 c3">par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.e.%20Grande%20Ouverture%20de%20Concert%2C%20Viola%20(280-329)%20obraz%20133b-141a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.f. </span>
        <span className="c1 c3">Grande Ouverture, Viola, Andante con moto,</span>
        <span className="c8 c3">&nbsp;</span>
        <span className="c1 c3">par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.f.%20Grande%20Ouverture%20de%20Concert%2C%20Viola%20(280-329)%20obraz%20141b-146.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.g. </span>
        <span className="c1 c3">Grand Ouverture de concert, Violoncello</span>
        <span className="c1">, </span>
        <span className="c1 c3">Andante con moto,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">par Jean Ruckgaber, </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.g.%20Grande%20Ouverture%20de%20Concert%2C%20Violoncello%20(280-329)%20obraz%20147-160.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c2 c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.h. </span>
        <span className="c1 c3">
          Grande Ouverture de Concert, Flauto, Andante con moto, par Jean
          Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.h.%20Grande%20Ouverture%20de%20Concert%2C%20Flauto%20(330-359)%20obraz%20329-333.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.i. </span>
        <span className="c1 c3">
          Grande Ouverture, Basso, Andante con moto, par Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c25 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.i.%20Grande%20Ouverture%20de%20Concert%2C%20Basso%20(330-359)%20obraz%20336-341.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.j. </span>
        <span className="c1 c3">
          Grande Ouverture, Basso, Andante con moto, par Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c25 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.j.%20Grande%20Ouverture%20de%20Concert%2C%20Basso%20(330-359)%20obraz%20348-355.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.k. </span>
        <span className="c1 c3">Grande Ouverture de Concert, Corno 3</span>
        <span className="c1 c11 c60">tia</span>
        <span className="c1 c3">
          &nbsp;in E, Andante con moto, par Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">3,4</span>
        <span className="c1 c10 c25">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.k.%20Grande%20Ouverture%20de%20Concert%2C%20Corno%203tia%20(330-359)%20obraz%20321-328.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c15">(Something wrong, three endings?)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">A.12.l. </span>
        <span className="c1 c3">
          Grande Ouverture de Concert, Basso, par Jean Ruckgaber,
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/A.12.l.%20Grande%20Ouverture%20de%20Concert%2C%20Basso%20(330-359)%20obraz%20342-347.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c6">&nbsp;</span>
      </p>

      <a className="section-ref" id="section-B" />
      <Typography variant="h3" gutterBottom>
        B. Music for a Chamber Orchestra
      </Typography>
      <p className="c0">
        <span className="c8 c3">B.1. Quintet pour piano D-dur, </span>
        <span className="c1 c3">op. 37, im Jahre 1815 </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">B.2.a.</span>
        <span className="c8 c15 c3">&nbsp;</span>
        <span className="c1 c15 c3">Premier Quatuer pour le Violon 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c15 c3">, 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1 c15 c3">, Viola et cello, op. 87</span>
        <span className="c1 c15">,</span>
        <span className="c8 c3">&nbsp;</span>
        <span className="c1 c15 c3">par Jean Ruckgaber, 7 April 1864, </span>
        <span className="c1 c15">ms. </span>
        <span className="c1 c11 c60">1,2,3 </span>
        <span className="c1 c10 c15">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/B.2.a.%20Premier%20Quatuer%20pour%20Violon%201mo%2C%202do%2C%20Viola%20et%20cello%20op.%2087%20%28366-368%29%20obraz%20108-134.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">B.2.b.</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c15 c3">1er Qua tuer pour Violon 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c15 c3">, Violon 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1 c15 c3">
          , Viola et Violoncello, par Jean Ruckgaber Op. 87
        </span>
        <span className="c1 c15">, </span>
        <span className="c1 c15 c3">Violoncello </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c15 c3">&nbsp;</span>
        <span className="c1 c10 c15">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/B.2.b.1.%20Premier%20Quarteur%20pour%20le%20Violon%201mo%2C%202do%2C%20Viola%20et%20Violoncello%2C%20op.87%2C%20Violoncello%20%28366-368%29%20obraz%20135-141.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c1 c15">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">B.2.c.</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">1er Qua tuer pour Violon 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, Violon 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1 c3">, Viola et Violoncello, Op. 87</span>
        <span className="c1">, </span>
        <span className="c1 c3">Viola </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/B.2.b.2.%20Premier%20Quatuer%20pour%20le%20Violon%201mo%2C%202do%2C%20Viola%20et%20Violoncello%2C%20op.%2087%2C%20Viola%20%28366-368%29%20obraz%20142-147.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">B.2.d.</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">1er Qua tuer pour Violon 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, Violon 2</span>
        <span className="c1 c11 c60">do</span>
        <span className="c1 c3">, Viola et Violoncello, Op. 87</span>
        <span className="c1">, </span>
        <span className="c1 c3">Violon 2do </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/B.2.b.3.%20Premier%20Quatuer%20pour%20le%20Violon%201mo%2C%202do%2C%20Viola%20et%20Violoncello%2C%20op.%2087%2C%20Violon%202do%20%28366-368%29%20obraz%20148-153.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c2 c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">B.2.e. </span>
        <span className="c1 c3">1er Qua tuer pour Violon 1</span>
        <span className="c1 c11 c60">mo</span>
        <span className="c1 c3">, Violon 2</span>
        <span className="c1 c11 c60">da</span>
        <span className="c1 c3">, Viola et Violoncello, Ouvre 87, Violon 1mo </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/B.2.c.%20Premier%20Quatuer%20pour%20le%20Violon%2C%20Violon%201mo%2C%20op.%2087%20%28366-368%29%20obraz%20154-162.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0 c21">
        <span className="c5 c3"></span>
      </p>

      <a className="section-ref" id="section-C" />
      <Typography variant="h3" gutterBottom>
        C. Music for the Piano
      </Typography>
      <p className="c0">
        <span className="c8 c3">C.1. Mazur pour piano D-dur, op. 1</span>
        <span className="c1 c3">, &agrave; M-lle Doris Renzhausen,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1, 2, 3, 7</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.1.%20Mazur%20D-dur%2C%20op.1%20%28280-329%29%20obraz%20296.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.2. Polonaise pour le pianoforte, a-moll, op. 2,
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3 c14">
          &agrave; M-lle Octavie de Festenburg, ca 1825,
        </span>
        <span className="c1 c14">&nbsp;ms. </span>
        <span className="c1 c3 c14">&nbsp;</span>
        <span className="c1 c11 c60">1,2,3,4, 6</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.2.%20Polonaise%20op.2%20%28280-329%29%20obraz%20529-530.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;(ms.), </span>
        <span className="c1 c11 c60">3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.2.a.%20Polonez%20op.%202%2C%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c8 c10">&nbsp;</span>
        <span className="c1">(print)</span>
        <span className="c1 c11 c60">6</span>
        <span className="c1 c10">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.3. Polonaise pour le pianoforte, d-moll, op. 3
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">d&eacute;di&eacute; &agrave; Jacques Bauer,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3,4,6</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.3.%20%28280-329%29%20obraz%20531-532.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">(ms.), </span>
        <span className="c1 c11 c60">3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.3.a.%20Polonaise%20op.3%2C%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c14">(print) </span>
        <span className="c1 c11 c60">6</span>
        <span className="c1 c10">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.4. Polonaise pour le pianoforte, f-moll, op. 4
        </span>
        <span className="c1">, </span>
        <span className="c1 c3 c14">
          d&eacute;di&eacute; &agrave; M-me la comtesse de
        </span>
        <span className="c30 c15 c3">&nbsp;</span>
        <span className="c1 c3 c14">Pieg&#322;owska,</span>
        <span className="c1 c14">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3,4,6</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.4.%20Polonaise%20op.4%20%28280-329%29%20obraz%20533-535.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">(ms.), </span>
        <span className="c1 c11 c60">3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.4.a.%20Polonaise%20op.4%2C%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c14">(print) </span>
        <span className="c1 c11 c60">6</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.5. III Cotillons pour le pianoforte </span>
        <span className="c1 c3">
          &agrave; M-me Ksawera Ruckgaber wyd. Piller &ndash; Lw&oacute;w, 1923
          r.
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">&nbsp;C.6. VII Cotillons, op. 5</span>
        <span className="c1">, </span>
        <span className="c1 c3">
          d&eacute;d. &agrave; M-lle Xavi&egrave;re de Rylska
        </span>
        <span className="c1 c11 c60">2,6 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.6.%20VII%20Cotillons%2C%20op.%205%2C%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.7. IX Variations pour le piano, op. 6,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">&agrave; M-elle Caroline de Rylska, </span>
        <span className="c1">ms. </span>
        <span className="c1 c11 c60">2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.7.%20IX%20Variations%2C%20op.%206%20%28366-368%29obraz291-304.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.8. </span>
        <span className="c8 c3 c14">
          Variations sur un th&egrave;me &bdquo;Robin de Bois&rdquo;, op.6,
        </span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c3 c14">pour piano, par Charles Marie Weber,</span>
        <span className="c1 c14">&nbsp;ms. </span>
        <span className="c1 c11 c60">2,3,4 </span>
        <span className="c1 c10 c25">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.8.%20Variations%20_Robin%20de%20Bois_%20op.6%20%28330-359%29%20obraz%20433-438.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c30 c42">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.9. Polonaise, op. 7</span>
        <span className="c1">,</span>
        <span className="c1 c3">
          &nbsp;d&eacute;di&eacute; &agrave; M-lle W. Schneikert
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.10. </span>
        <span className="c8 c3 c14">
          Collection de 4 polonaises, 10 valses avec trio, 1 quadrille, 2
          galloppes et 5 cotillons, op. 8
        </span>
        <span className="c1 c14">, </span>
        <span className="c1 c3">par J. Ruckgaber et J. Baschny, 1926 r.</span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.11. Charme du carnaval, op. 9</span>
        <span className="c8">, </span>
        <span className="c1 c3">
          d&eacute;di&eacute; &agrave; M-lle comte Hortense Ma&#322;achowska,
        </span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">2,6 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.11.%20Ta%C5%84ce.%20Op.%209%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.12. Grand marche a 4 mains, op. 10</span>
        <span className="c1">, </span>
        <span className="c1 c3">
          &agrave; monsieur le comte de Fresnel, g&eacute;n&eacute;ral de la
          chevalierie, commandant g&eacute;n. en Galizie,
        </span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">2, 3, 4, </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.12.a.%20Grand%20marche%20a%204%20mains%2C%20op.%2010%20(330-359)%20obr.%2056b%2C%20(280-329)%20obr.057-063.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">, </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.12.b.%20Grand%20marche%20a%204%20mains%2C%20op.%2010%20(280-329)%20obraz%20064-067.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.13. Exercises de danses de l&rsquo;ann&eacute;e 1826, op. 11,
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">
          compos&eacute;es par A. Braun et J. Ruckgaber{' '}
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.14. Collection des danses, op. 12, </span>
        <span className="c1 c3">
          d&eacute;di&eacute;e &agrave; M-lle la comtesse Eveline Dzieduszycka
        </span>
        <span className="c1 c11 c60">&nbsp;2,8</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.14.%20Valses_Mazures_et_Galoppes%2C%20BJ.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.15. 2 polka, op. 12 </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.16. </span>
        <span className="c8 c3 c14">
          Variations sur un th&egrave;me russe, op. 12,
        </span>
        <span className="c1 c14"> </span>
        <span className="c1 c15 c3">
          piano &agrave; M-lle la comtesse Angelique
        </span>
        <span className="c30 c25 c3">&nbsp;</span>
        <span className="c1 c15 c3">Morszkowska, </span>
        <span className="c1 c14">ms. </span>
        <span className="c1 c11 c60">1,2,3 </span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.16.%20Variations%20sur%20un%20th%C3%A8me%20russe%2C%20op.%2012%20(366-368)%20obraz%20484-490a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.17.</span>
        <span className="c8 c3 c14">
          &nbsp;No 2 Grande marche pour le pianoforte &agrave; 4 mains, op. 12
        </span>
        <span className="c1 c14">,</span>
        <span className="c1 c3 c14">&nbsp;compos&eacute;e par J. Ruckgaber </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.17.%20No%202%20Grande%20marche%20%20pour%20le%20pianoforte%20%C3%A0%204%20mains%2C%20op.12%20(366-368)%20obraz%20490b-492.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c2 c1">&nbsp; </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.18. Exercises de danses, op. 13</span>
        <span className="c1">, </span>
        <span className="c1 c3">
          d&eacute;d. &agrave; M-elle Melanie de Uruska{' '}
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.19. Eugenie &ndash; polka, op. 13</span>
        <span className="c1">, </span>
        <span className="c1 c3">&agrave; M-elle Eugenie Mehlieu, </span>
        <span className="c1">ms. </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.20. Mazur &ndash; Favorit, op. 14</span>
        <span className="c1">, </span>
        <span className="c1 c3">
          &agrave; M-elle H&eacute;l&egrave;ne de Gregoreska
        </span>
        <span className="c1 c11 c60"> 2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.21. Fantasie sur un th&egrave;me de l&rsquo;op&eacute;ra, &bdquo;La
          dame blanche&rdquo;,G-moll, op. 14
        </span>
        <span className="c1">, piano, ms. </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.22. Fantaisie tir&eacute;e de l&rsquo;op&eacute;ra &bdquo;Le dame
          blanche&rdquo;, op. 15
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">&agrave; M-me Louise Piller</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.23. Exercices de danses pour le pianoforte, op.15,
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">ca 1830 </span>
        <span className="c1 c11 c60">6</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.23.%20Ta%C5%84ce.%20Op.%2015%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">
          Polonaise No 1 D-dur; Polonaise No 2 E-dur;
        </span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">
          Walce: No 1 D-dur; No 2 G-dur; No 3 B-dur; No 4 F-dur &quot;La dame
          blanche&quot;; No 5 D-dur;
        </span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">
          Mazur No 1 F-dur; Mazur No 2 D-dur / par M. Br...ski;
        </span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">Galoppe D-dur;</span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">Anglaises: No 1 D-dur; No 2 D-dur;</span>
      </p>
      <p className="c0">
        <span className="c1 c3">Cotillion: No 1 D-dur; No 2 A-dur No 3 D-dur</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.24. Marche de Mo&iuml;se vari&eacute;e, op. 15{' '}
        </span>
        <span className="c1 c11 c60"> 3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.24.%20Notatki%20%28%28330-359%29%20obraz%20514.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">( notes only)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.25. Polonaise pour pianoforte, D-dur, op. 16,{' '}
        </span>
        <span className="c1 c11 c60">1,3,4,6</span>
        <span className="c1 c10">
          &nbsp;{' '}
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.25.%20Polonaise%20op.16%2C%20manuscript%2C%20%28280-329%29%20obraz%20536-538.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;(ms.), </span>
        <span className="c1 c11 c60">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.25.a.%20Polonaise%20op.16%2C%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;(print) </span>
        <span className="c33 c8 c13 c23 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.26. Valses, op. 17</span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp; piano, ca 1835 </span>
        <span className="c1 c11 c60">2,6 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.26%20Valses%20op.17%2C%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.27. Grande Marche, op. 17</span>
        <span className="c1 c3">&nbsp;&agrave; 4 mains, </span>
        <span className="c1">ms.</span>
        <span className="c1 c11 c60">1,2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.27.%20Grande%20Marche%20a%204%20mains%2C%20op.17%20(330-359)%20obraz%20048b-050.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.28. Rondo sur les motifs de &bdquo;Zalmira&rdquo; par Rossini, op.
          18,
        </span>
        <span className="c1 c3">&nbsp;piano, 1866, Rondo 1824,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3 &nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.28.%20Rondo%20sur%20les%20motifs%20de%20%E2%80%9EZalmira%E2%80%9D%2C%20op.%2018%2C%28330-359%29%20obraz%20284b-304.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c8 c3">&nbsp;</span>
        <span className="c1 c14">(+ bas, tenor 1mo, 2do?)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.29. Polonaise, mazures, cotillions, valse et galloppe - pour le
          Pianoforte, op. 18{' '}
        </span>
        <span className="c1 c11 c60">6</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.29.%20Polonaise%2C%20mazures%2C%20cotillions%2C%20valse%20et%20galloppe%2C%20op.%2018%2C%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c2 c1">Polonaise Es-dur;</span>
      </p>
      <p className="c0">
        <span className="c2 c1">
          Masure No 1 A-dur; No 2 F-dur; No 3 D-dur ; No 4 D-dur ; No 5 D-dur;
          No 6 D-dur
        </span>
      </p>
      <p className="c0">
        <span className="c2 c1">
          Cotillions No 1 D-dur; No 2 D-dur; No 3 D-dur; No 4 G-dur; No 5 D-dur;
          No 6 D-dur; No 7 D-dur;
        </span>
      </p>
      <p className="c0">
        <span className="c2 c1">Valse D-dur;</span>
      </p>
      <p className="c0">
        <span className="c1">Galloppe c-moll.</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.30. Grandes variationes sur un th&egrave;me du ballet
          &bdquo;Nina&rdquo; F-dur, op. 19
        </span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;piano, </span>
        <span className="c1">ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.30.%20Grandes%20variationes%20sur%20un%20th%C3%A8me%20du%20ballet%20%E2%80%9ENina%E2%80%9D%2C%20op.%2019%20(366-368)obraz507-514a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c34">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.31. Polonaise pour le pianoforte, op. 19</span>
        <span className="c1">, </span>
        <span className="c1 c3">ca 1835 </span>
        <span className="c1 c11 c60">2,6 </span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.31.a.%20Polonaise%20op.%2019%2C%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c6">, </span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.31.b.%20Polonaise%20op.19%2C%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.32. </span>
        <span className="c8 c3 c14">
          Rondo sur un th&egrave;me de l&rsquo;op&eacute;ra
          &bdquo;Zalmira&rdquo; de Rossini, pour piano, op. 20
        </span>
        <span className="c1 c14">,</span>
        <span className="c30 c15">&nbsp;</span>
        <span className="c1 c3 c14">1824</span>
        <span className="c1 c14">, </span>
        <span className="c1 c25">ms. </span>
        <span className="c1 c11 c60">1, 2, 3, 4, </span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.32.%20Rondo%20sur%20un%20th%C3%A8me%20de%20l%E2%80%99op%C3%A9ra%20%E2%80%9EZalmira%E2%80%9D%20op.%2020%2C(330-359)%20obraz%20263-270.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c14 c34">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.33. Mazur et Galoppe, op. 21</span>
        <span className="c1">,</span>
        <span className="c1 c3">
          &nbsp;&agrave; M-me Caroline ...bowska n&eacute;e de Balewska
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.34. Musique pour les amusements des danses, op. 22
        </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.35. Musique pour les amusements des danses, op. 23
        </span>
        <span className="c1 c11 c60">2</span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.36. Fantaisie sur les th&egrave;mes de l&rsquo;op&eacute;ra
          &bdquo;Il Graciono&rdquo; de Meyerbeer, op. 24
        </span>
        <span className="c1">, 1837 </span>
        <span className="c1 c11 c60">2,7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.37. M&eacute;lange tir&eacute; de l&rsquo;op&eacute;ra &bdquo;Fra
          Diavolo&rdquo;, op. 25{' '}
        </span>
        <span className="c1 c11 c60">1,2</span>
        <span className="c8 c13 c23 c3 c33">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.38. L&rsquo;agr&eacute;ment du Carnaval, op. 27,
        </span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c3 c14">
          Es-Dur, A-Dur, D-Dur, D-Dur, c-Moll, F-Dur, 1858{' '}
        </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.39. Lwowianka, mazur, op. 27</span>
        <span className="c1">, </span>
        <span className="c1 c3">d&eacute;d. &agrave; M. Vincent Daner </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C. 40. Polonaise, compos&eacute;e a Husiatyne en Pologne,
        </span>
        <span className="c1 c3">&nbsp;1840,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C. 41. Lieblings-Mazuren Ihrer Majest&auml;t der Kaiserin von
          Russland.{' '}
        </span>
        <span className="c1 c11 c60">7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.42. &nbsp;Mazurka, op. 28</span>
        <span className="c8">,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">
          piano &agrave; M-me Comtesse Haeltenschmidt (?){' '}
        </span>
        <span className="c1 c11 c60">2,7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.43. M&eacute;lange sur diff&eacute;rentes Th&egrave;mes Polonais
          pour Piano Forte &agrave; 8 mains, op. 29
        </span>
        <span className="c1 c11 c60">1,2,3,4,5,7</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.43.c.%20M%C3%A9lange%20%C3%A0%208%20mains%2C%20op.%20291(280-329)%20obraz%20335-(brak%20str.350)-366%2Crkp.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">3 </span>
        <span className="c1">&nbsp;(ms.), </span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.43.d.%20Melange%20op.%2029%2C%20druk.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
        <span className="c1">&nbsp;(print) </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.44. Duo &agrave; 8 mains ou pour 4, piano, op. 29
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.45. Rondo de concert pour piano, op. 30 </span>
        <span className="c1 c11 c60">&nbsp;3, 4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.45.%20Rondo%20de%20concert%20pour%20piano%2C%20op.%2030%20%28330-359%29%20obraz%20305-309.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.46. Variations sur une Walse, op. 30</span>
        <span className="c8">,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">1838 </span>
        <span className="c1 c11 c60">2,5,7</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.46.a.%20Variations%20sur%20une%20Walse%20op.%2030.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
        <span className="c1">, </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.46.b.%20Variations%20sur%20une%20Walse%20Op.%2030%20Marie%20Kratter%20BN%20obr.1-22.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.47. Poutpourri tir&eacute;e de l&rsquo;op&eacute;ra
          &bdquo;Belizario&rdquo; par Donizetti, op. 30
        </span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c3 c14">1863, </span>
        <span className="c1 c14">ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.47.a.%20Poutpourri%20tir%C3%A9e%20de%20l%E2%80%99op%C3%A9ra%20%E2%80%9EBelizario%E2%80%9D%20par%20Donizetti%2C%20op.%2030%20(280-329)%20obraz539-547.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">, </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.47.b.%20Poutpourri%20tir%C3%A9e%20de%20l%E2%80%99op%C3%A9ra%20%E2%80%9EBelizario%E2%80%9D%20par%20Donizetti%2C%20op.%2030%20(280-329)%20obraz%20548-558.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.48. Mazurka et 6 quadrilles, op. 31</span>
        <span className="c1 c3">
          , &agrave; M-me la comtesse Julie G&#322;ogowska
        </span>
        <span className="c1 c11 c60">2</span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.49. Variations sur un th&egrave;me original pour le pianoforte, op.
          32
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">1836 </span>
        <span className="c1 c11 c60">2,4,5,7 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.49.%20Variations%20op.%2032.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
        <span className="c1 c13">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.50. Variations sur un th&egrave;me de l&rsquo;op&eacute;ra
          &bdquo;Norma&rdquo; de Bellini, op. 33
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">February 1837 </span>
        <span className="c1 c11 c60">2,7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.51. Air Polonais vari&eacute;, op. 34 </span>
        <span className="c1">,</span>
        <span className="c1 c3">
          &nbsp;&agrave; M-me la comtesse Leonore Karwicka, 1836{' '}
        </span>
        <span className="c1 c11 c60">2,7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.52. Souvenir de Bellini, op. 35</span>
        <span className="c1">, </span>
        <span className="c1 c3">
          &agrave; M-lle la comtesse Justine Markowska, 1836{' '}
        </span>
        <span className="c1 c11 c60">2,7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.53. Souvenir de Hugenots de Meyerbeer, op. 36, Phantasie,{' '}
        </span>
        <span className="c1 c3">1837 </span>
        <span className="c1 c11 c60">2,7</span>
      </p>
      <p className="c0">
        <span className="c8 c3 c14">
          C.54. Pr&eacute;lude et Pri&egrave;re pour l&rsquo;orgue ou piano, op.
          36
        </span>
        <span className="c1">,</span>
        <span className="c1 c3 c14">&nbsp;</span>
        <span className="c1 c15 c3">
          &agrave; son ami M-sieur Victor de Sellyey,
        </span>
        <span className="c1 c3 c12">&nbsp;</span>
        <span className="c1 c15">ms. </span>
        <span className="c1 c11 c60">1,2,3 &nbsp;</span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.54.%20Pr%C3%A9lude%20et%20Pri%C3%A8re%20pour%20l%E2%80%99orgue%20ou%20piano%2C%20op.%2036%20(366-368)obraz187-190.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3 c14">&nbsp;</span>
        <span className="c8 c3">
          C.55. Beaut&eacute;s de &bdquo;Huguenots&rdquo; de Meyerbeer, op. 37
        </span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;piano </span>
        <span className="c1 c11 c60">2</span>
        <span className="c1 c13">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.56. Fantasie ou Potpourri, op. 38, </span>
        <span className="c1">1838 </span>
        <span className="c1 c11 c60">2,7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.57. 9 Mazur&oacute;w, op. 39</span>
        <span className="c1">, </span>
        <span className="c1 c3">
          d&eacute;d. &agrave; M-me Honorie de Komorowska n&eacute;e comtesse
          Chotoniewska
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.58.a. Ouvertur zur komischen Oper &bdquo;Der Doctor und
          Apotheker&rdquo;, op. 40
        </span>
        <span className="c1 c15">,</span>
        <span className="c1 c15 c3">&nbsp;zu 4 h</span>
        <span className="c15 c3">&auml;</span>
        <span className="c1 c15 c3">nde</span>
        <span className="c1 c15">, ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1 c15">&nbsp;</span>
        <span className="c1 c10 c15">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.58.a.%20Ouvertur%20zur%20komischen%20Oper%20%E2%80%9EDer%20Doctor%20und%20Apotheker%E2%80%9D%2C%20op.%204040(280-329)%20obraz%20470-480.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.58.b. Ouvertur de L&rsquo;Opera &ldquo;L&rsquo;docteur et
          Pharmacien&rdquo; a 4 mains, op. 40
        </span>
        <span className="c1 c11 c60">2,3</span>
        <span className="c1 c10 c15">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.58.a.%20Ouvertur%20zur%20komischen%20Oper%20%E2%80%9EDer%20Doctor%20und%20Apotheker%E2%80%9D%2C%20op.%2040%20(280-329)%20obraz%20481-495.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.59. 6 Quadrilles na piano, op. 40</span>
        <span className="c8">,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">&agrave; M-me Amalie Eder </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.60. Concertus ....pour le piano, op. 40 </span>
        <span className="c1 c11 c60">4</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.61. 3 Mazures &agrave; Mons. Fr. Chopin, op. 41
        </span>
        <span className="c1 c11 c60">1,2</span>
        <span className="c8 c3">&nbsp;(op. 43 </span>
        <span className="c1 c11 c60">2</span>
        <span className="c8 c3">?)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.62. Polonaise et 3 mazures, op. 42 </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.63. 3 Mazures et 6 Quadrilles, op. 45 </span>
        <span className="c1 c11 c60">1,2,5 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.63.%203%20Mazures%20et%206%20quadrilles%20Op.45.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.64. Cantus Firmus, op. 45 </span>
        <span className="c1 c11 c60">2,3,4</span>
        <span className="c1">, </span>
        <span className="c1 c15">
          (with double counterpoint for 3 voices, 1860, ,, musical
          curiosum&rdquo;){' '}
        </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.64.%20Cantus%20Firmus%2C%20op.%2045%20(330-359)%20obraz%20021-024.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.65. 3 stimiger Canon in strangen Stil,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">25 Nov. 1865</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">2,3 </span>
        <span className="c9 c1">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.65.%203%20Stiminger%20Canon%20(366-368)%20obr.%20274-275.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.66. &bdquo;Pod gwiazdami&rdquo;, op. 46 </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.67. Nocturne, op. 47, </span>
        <span className="c1 c3">
          fortep. d&eacute;di&eacute; &agrave; mon ami, Artur G&#322;ogowski{' '}
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.68. 3 Fuges en mi- mineur, re- mineur et mi- mineur et cantus
          firmus, contrepoint &agrave; 3 voix pour piano, op. 48,{' '}
        </span>
        <span className="c1 c3">&agrave; son ami Monsieur Francois Candella </span>
        <span className="c1 c11 c60">2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.68.%203%20Fuges%20op.48%20(280-329)%20obraz%20028b-033a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.69. Marche fun&egrave;bre pour le pianoforte, op. 48
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">Souvenir &agrave; Th. Wis&#769;niowski </span>
        <span className="c1 c11 c60">2,5,7 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.69.%20Marche%20Funebre%20op.%2048.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.70. Andante pour le pianoforte As-dur, op. 48
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1">&nbsp; </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.70.%20Andante%20op.48%20%28330-359%29%20obraz%20026-038.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.71. </span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c8 c3">Scherzo op.50</span>
        <span className="c1 c3">, manuscript </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1">&nbsp; </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.71.%20Scherzo%20op.%2050%20%28330-359%29%20obraz%20039b-048a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.72. Souvenir de l&rsquo;op&eacute;ra &bdquo;Mina&rdquo; de Thomas,
          op. 49
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">
          d&eacute;d. &agrave; M-me Victorine de Vassilka
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.73. Fuga en sol-mineur pour piano, op. 50</span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;&agrave; M-sieur Jacques de Kobervein, </span>
        <span className="c1">ms. </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c5 c3">
          C.74. Sentiments d&rsquo;enthousiasme de la Galicie pour les
          m&eacute;morables journ&eacute;es
        </span>
        <span className="c8 c3">d&rsquo;octobre de 1851, op. 50 </span>
        <span className="c1 c11 c60">5 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.74.%20Sentiments%20op.50.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C. 75. Canon &agrave; deux voix pour piano, op. 50,{' '}
        </span>
        <span className="c1 c3">ms. </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C. 76. 5 valses, 2 mazures et une polka pour piano, op. 51
        </span>
        <span className="c1 c11 c60">2,5</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.76.%205%20Valses%2C%202%20Mazures%20et%20une%20Polka%20op.%2051.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.77. Quadrilles sur des airs favoris de l&rsquo;op&eacute;ra
          &bdquo;Indra&rdquo; de Flotow, op. 52
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.78. Penices romans sans paroles, op. 53</span>
        <span className="c1">,</span>
        <span className="c1 c3">
          &nbsp;d&eacute;d. &agrave; M-lle Olga de Zawadzka
        </span>
        <span className="c1 c11 c60">1,2,7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.79. 3 mazures, op. 54</span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;&agrave; son ami Edmond Juristowski </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.80. Fuga in 4 parties ... pour le Piano Forte, op. 54
        </span>
        <span className="c1 c11 c60">2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.80.%20Fuga%20in%204%20parties%20op.54%20(280-329)%20obraz%20015-019.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.81. Fuga &agrave; 3 parties en mi-maieur, pour piano, op. 55
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.81.%20Fuga%20%C3%A0%203%20parties%20op.55%20(280-329)%20obraz%20022b-024.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.82. Mazur &agrave; 6 mains D-dur, op. 56 </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.82.%20Mazur%20%C3%A0%206%20mains%2C%20op.%2056%20(280-329)%20obraz%20298-305.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.83. Grande marche pour le piano, No 6, op. 56
        </span>
        <span className="c1 c11 c60">1,2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.83.a.%20Grande%20marche%2C%20op.%2056%20(330-359)%20obraz%20068-072.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">
          &nbsp;(manuscript pages numbered from page 20?),{' '}
        </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.83.b.%20Grande%20Marche%20No%206%2C%20op.%2056%20(280-329)%20obr.%20068-070.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.84. &bdquo;Souvenir de Korczyn&rdquo;, op. 57
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">dans les Karpathes, Mazur </span>
        <span className="c1 c11 c60">2</span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.85. Szamszurka &ndash; polka, op. 58 </span>
        <span className="c1 c11 c60">1,2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.85.b.%20Polka%20-%20Szamszurka%20op.104%20(280-329)%20obraz%20526-528.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;(a 4 mains, op. 104) </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.86. Polka &ndash; mazurka, op. 59 </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.87. Impromptu, op. 59 </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.88. Cavatina et andante de l&rsquo;op&eacute;ra
          &bdquo;Zalmira&rdquo; &agrave; 4 mains, op. 59
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2</span>
        <span className="c1 c11">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.89. Frantzl &ndash; Polka et Mazur tremblante, op. 59
        </span>
        <span className="c1">, </span>
        <span className="c1 c25 c3">
          piano &agrave; M-me Rose de Dobrzycka, 1859,{' '}
        </span>
        <span className="c1 c25">ms. </span>
        <span className="c1 c11 c60">1,2,3,4 </span>
        <span className="c1 c10 c25">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.89.%20Frantzl%20%E2%80%93%20Polka%20et%20Mazur%20tremblante%2C%20op.%2059%20(280-329)%20obraz%20013-014.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.90. Mazurca de salon, op. 60 </span>
        <span className="c1 c11 c60">2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.90.%20Mazurca%20de%20salon%2C%20op.%2060%20(366-368)obraz218-222.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.91. Polka, op. 61 </span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;&agrave; M-lle Maria Lewakowski </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.92. Mazurka pour le piano, op. 62 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.92.%20Mazurka%20pour%20le%20piano%2C%20op.%2062.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.93. Fleurs de Korczyn, Quadrilles pour le piano, op. 63
        </span>
        <span className="c1 c11 c60">2,5</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.93.%20Fleurs%20de%20Korczyn%2C%20op.%2063.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c2 c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.94.</span>
        <span className="c1">&nbsp;</span>
        <span className="c8 c3">
          Ouverture de m&eacute;lodrama &bdquo;Le Sauveur&rdquo;, op. 63,
        </span>
        <span className="c1 c3">
          &nbsp;arrang&eacute;e pour piano &agrave; 4 mains,
        </span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c2 c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.94.c.%20%28330-359%29%20obraz%20439%20ok%C5%82adka.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;cover</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.94.d.%20%28330-359%29%20obraz%20001%20strona%20tytu%C5%82owa.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;title page (op. 53!?)</span>
        <span className="c18 c1 c11">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.94.e.%20%28280-329%29%20obraz496-%28brak%20506%2C507%2C%20tj.%20str.20-21%2C%2022-23%20rkp.%29-509.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">(missing pages 20-21, 22-23 ms.), (op. 53!?)</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.94.f.%20Ouverture%20de%20melodrama%20_Le%20Sauveur_%2C%20op.36%20%28330-359%29%20obraz%20002-013%2C%20kompletny.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp; (op.36!?)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.95. Grande Ouverture pour grande Orchestre, arrangee pour piano a 4
          mains, op. 63
        </span>
        <span className="c1 c3">, ms. </span>
        <span className="c1 c11 c60">2,3,4</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.95.%20Grande%20Ouverture%20a%204%20mains%2C%20op.63%20%28366-368%29obraz516-537.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">(see also A.12, version for orchestra)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.96. Une fleur de salon &ndash; Impromptu-valse, op. 64
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">
          d&eacute;d. &agrave; M-sieur Antoine Kocipin&#769;ski
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.97. Grande marche pour piano, op. 64</span>
        <span className="c1">, </span>
        <span className="c1 c3">au Corps des Officiers du Regiment Nr. 65 </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.97.%20Grande%20marche%20pour%20piano%2C%20op.%2064%20%28280-329%29%20obraz%20071-073.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.98. 4-ti&egrave;me fuge &agrave; 3 voix pour pianoforte, op. 64
        </span>
        <span className="c8">, </span>
        <span className="c1">ms. </span>
        <span className="c1 c11 c60">2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.98.%204-ti%C3%A8me%20fuge%20%C3%A0%203%20voix%2C%20op.%2064%20(280-329)%20obraz%20025-028a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.99</span>
        <span className="c8">.</span>
        <span className="c1">&nbsp;</span>
        <span className="c8 c3">Grande ouverture op. 65</span>
        <span className="c1">&nbsp;(?),</span>
        <span className="c1 c3">
          &nbsp;pour grand orchestre, arrang&eacute;e pour 4 mains, piano,
        </span>
        <span className="c2 c1">&nbsp;</span>
        <span className="c1">
          (Overture for the opening of the Skarbkowski Theater, composed in
          Korczyn in August 1856)
        </span>
        <span className="c1 c11 c60">1,2 </span>
        <span className="c1">
          &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.100. </span>
        <span className="c8 c3 c14">
          4 Mazurka &eacute;l&eacute;gante, piano, B-dur, op. 65,
        </span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c15 c3">
          {' '}
          &agrave; son ami le Chevalier Marcelle Madeyski,
        </span>
        <span className="c1 c15">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3,4 </span>
        <span className="c1 c10 c15">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.100.%204%20Mazurka%20%C3%A9l%C3%A9gante%2C%20op.%2065(280-329)%20obraz315b-322a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.101. Impromptu, sc&egrave;ne dramatique pour piano, op. 65
        </span>
        <span className="c1">, 1861, </span>
        <span className="c1 c11 c60">1,2,3,5 </span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.101.a.%20Impromptu%2C%20sc%C3%A8ne%20dramatique%20pour%20piano%2C%20op.%2065.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
        <span className="c1">&nbsp;(print) </span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.101.b.%20Impromptu%2C%20op.%2065(366-368)obraz%20555-559a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;(ms.) </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.102. Souvenir des Karpathes, op. 65 </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.102.%20Souvenir%20des%20Karpathes%2C%20op.%2065%20(366-368)obraz%20269b-273.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.103. Cavatine pour piano par Jean Ruckgaber, op. 66
        </span>
        <span className="c1 c11 c60">2,5,7</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.103.%20Cavatine%20op.66.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.104. Souvenir de Chervitschesti, 6 Quadrilles pour piano, op. 66
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.104.%20Souvenir%20de%20Chervitchesti%2C%20op.66%20(366-368)obraz225-229.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.105. Souvenir de belles Dames de Dombrowa, Polka Nr.4,
        </span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.105.%20Souvenir%20de%20belles%20Dames%20de%20Dombrowa%2C%20Polka%20Nr.4%20(366-368)obraz%20223-224.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.106. Reconaisence &ndash; Polka No.5, Op. 67
        </span>
        <span className="c1 c11 c60">2,3</span>
      </p>
      <p className="c0">
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.105.1.%20Johannis%20Brunn%20et%20Stefanie%20Polka%20No.11%2C12%2C%20Op.%2067%20(366-368)obraz%20574-576.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">Johannis Brunn et Stefanie</span>
        <span className="c1 c10">&nbsp;</span>
        <span className="c1">Polka No.11,12, Op. 67 </span>
        <span className="c1 c11 c60">3</span>
        <span className="c27 c1 c28 c11 c25">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.106.2.%20Polka%20No%202%20(366-368)%20obr.%20316-317.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c6 c34">&nbsp;</span>
        <span className="c6">Helene Polka No 2 </span>
        <span className="c1 c11 c60">3</span>
      </p>
      <p className="c0">
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.106.4.%20Polka%20No%204%20(366-368)%20obr.%20318-319.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c6">&nbsp;Polka No 4 </span>
        <span className="c1 c11 c60">3</span>
      </p>
      <p className="c0">
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.106.5.%20Reconaisence%20%20Polka%20No%205(366-368)obraz095-097.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c6">&nbsp;Reconaisence Polka No 5 </span>
        <span className="c1 c11 c60">3</span>
      </p>
      <p className="c0">
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.106.6.%20Jeanne%20Polka%20No%206%20(366-368)%20obr.%20313-314.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c6">Jenne Polka No 6 </span>
        <span className="c1 c11 c60">3</span>
      </p>
      <p className="c0">
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.106.7.%20Emilie%20Polka%20No%207%20(366-368)%20obraz%20315.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c6">Polka No 7 </span>
        <span className="c1 c11 c60">3</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.107. 3 Polka op. 67 </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.107.%20%203%20Polka%20op.%2067%20(280-329)%20obraz%20522-525.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.108. Jeanette &ndash; Polka, Nr. 8 </span>
        <span className="c1 c11 c60">2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.108.%20Jeanette%20%E2%80%93%20Polka%2C%20Nr.%208%20(366-368)%20obraz%20572.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.109. &ldquo;Qui ne s&rsquo;oublie pas</span>
        <span className="c8">&rdquo; </span>
        <span className="c8 c3">Polka Nr. 16 </span>
        <span className="c1 c11 c60">2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.109.%20%E2%80%9CQui%20ne%20s%E2%80%99oublie%20pas%E2%80%9D%20Polka%20Nr.%2016%20(366-368)obraz%20093-094.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.110. Raoul &ndash; polka, op. 67 </span>
        <span className="c1 c11 c60">2,5,7 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.110.%20Raoul-Polka%20Op.%2067.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.111. Les plus beaux yeux: m&eacute;lodie de Stigelli, op. 67
        </span>
        <span className="c8">,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">ca 1885 (???) </span>
        <span className="c1 c11 c60">6</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.112. Le Tra&icirc;nage, polka, op. 68 </span>
        <span className="c1 c11 c60">2</span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.113. Grande polonaise A-moll, op. 68 </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.114. 7 mazur&oacute;w na korzys&#769;c&#769; Zak&#322;adu
          G&#322;uchoniemych, op. 69
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.115. Canon op. 69</span>
        <span className="c1">, </span>
        <span className="c1 c3">piano</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.116. Nocturno pour piano, op. 70 </span>
        <span className="c1 c11 c60">2,3,4,5 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.116.a.%20Nocturno%20Op.70.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
        <span className="c1">&nbsp;(print), </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.116.b.%20Nocturno%20op.70%20(330-359)%20obraz%20430-432.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
          {' '}
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1">&nbsp;(ms.) </span>
       </p>
       <p className="c0">
        <span className="c8 c3">
          C.117. Souvenir de Miroslawa, op. 71, 1) Scherzo 2) Mon desir
        </span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;1862 </span>
        <span className="c1 c11 c60">2,5,7</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.117.%20Souvenir%20de%20Miroslawa%20Op.71.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.118. 4-me Etude pour Piano, op. 71</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.119. 6 Etudes pour piano, op. 72</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.120. Le dernier des hussards, r&ecirc;verie, op. 72
        </span>
        <span className="c1 c11 c60">1,2,5,7 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.120.%20Le%20Dernier%20des%20Hussards%20Oeuvre%2072.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
      </p>
      <p className="c0">
        <span className="c5 c3">
          C.121. Olla Potrida - Contredanses Tir&eacute;es de
          l&rsquo;op&eacute;ra Stradella et le Cheval de
        </span>
        <span className="c8 c3">Bronze&rdquo;, op. 73</span>
        <span className="c8">,</span>
        <span className="c1 c3">
          &nbsp;&agrave; M-lle la comt. S&eacute;v&eacute;rine Siemin&#769;ska
        </span>
        <span className="c1 c11 c60">2,8 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.121.%20Olla_Potrida_op.73%2C%20BJ.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.122. Valse brillante, A-dur, op.73,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.122.%20Valse%20brillante%2C%20op.73(366-368)obraz286-290.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.123. 6 Etudes pour piano, op. 73,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.124. 2 Etudes pour piano, op. 74,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.125. Souvenir des Karpathes, Pens&eacute;es megoriques et
          nationales, op. 74,{' '}
        </span>
        <span className="c1">ms. </span>
        <span className="c1 c11 c60">1,2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.125.%20Souvenir%20des%20Karpathes%2C%20op.74%20(366-368)obraz%20265-269a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.126. Fleurs Roumaines, op. 75</span>
        <span className="c8">,</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c3 c14">Transcription pour le piano par Jean</span>
        <span className="c15 c3 c30">&nbsp;</span>
        <span className="c1 c3 c14">Ruckgaber, 1863 </span>
        <span className="c1 c11 c60">1,3,4</span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.126.%20Fleurs%20Roumaines%2C%20op.%2075%20(280-329)%20obraz%2005-012.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.127. Marche Favorite Vari&eacute;e Pour Le Pianoforte a 4 Mains, Op.
          75{' '}
        </span>
        <span className="c1 c11 c60">2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.127.%20Marche%20Favorite%20%20a%204%20Mains%2C%20Op.75%20(280-329)%20obraz%20270-278.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.128. Mazurka pour le piano &agrave; 4 mains C-Dur, op. 75
        </span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;par Jean Ruckgaber </span>
        <span className="c1 c11 c60">1,2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.128.%20Mazurka%20%C3%A0%204%20mains%2C%20op.75%20(280-329)%20obraz322b-327a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.129. Salut &agrave; la Roumanie, op. 76</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.129.%20Salut%20a%20la%20Roumanie%2C%20op.76%20(280-329)%20obraz%2004%2C%20tylko%20ok%C5%82adka.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c2 c1">(cover only)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.130. Pens&eacute;e fugitive, souvenir de Skee, B-Dur, op. 76
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.130.%20Pens%C3%A9e%20fugitive%2C%20op.76%20(280-329)%20obraz516-521.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.131. Le dernier r&ecirc;verie de Hussard, op. 76
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.132. Mondnacht auf der Alpe, H-Dur, op. 77 </span>
        <span className="c1 c11 c60">1,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.132.%20Mondnacht%20auf%20der%20Alpe%2C%20op.77%20(280-329)%20obraz419-423a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.133. Suite des masures pour piano, op. 77 </span>
        <span className="c1 c11 c60">1,2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.133.%20Suite%20des%20masures%20pour%20piano%2C%20op.%2077%20(366-368)%20obraz%20276-279.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.134. Nachtfahrt um K&ouml;nigssee, nocturne, op. 78
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.134.%20Nachtfahrt%20um%20K%C3%B6nigssee%2C%20op.%2078%20(280-329)%20obraz%20423b-426.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3 c14">C.135. </span>
        <span className="c8 c15 c3">
          Doppel Fuge im 4 stimmigen Kontrapunkt f&uuml;r Piano oder Orgel zu 4
          H&auml;nde, op. 78,
        </span>
        <span className="c1 c15">&nbsp;</span>
        <span className="c1 c15 c3">1863,</span>
        <span className="c1 c15">&nbsp;ms. </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.136. 4-&egrave;me mazurka de salon, pour piano, op. 78
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.136.a.%20Mazurka%20de%20Salon%20a%204%20mains%2C%20op.%2078%20(280-329)%20obr.%20310-315a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
          ,
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.136.b.%204-%C3%A8me%20mazurka%20de%20salon%2C%20op.78%20(330-359)%20obraz%20310-320.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.137. Nachtfahrt,</span>
        <span className="c8">&nbsp;</span>
        <span className="c8 c3">op. 79 </span>
        <span className="c1 c11 c60">2,3,4 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.137.%20Nachtfahrt%2C%20op.%2079%20(280-329)%20obraz%20427-430.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.138. Mazurka, op. 79 </span>
        <span className="c1 c11 c60">2,8</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.138.%20Mazurka%20op.79%2C%20BJ.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.139. Pauline-polka pour piano, Nr 10, op. 79,
        </span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.139.a.%20Pauline-polka%20pour%20piano%2C%20Nr%2010%2C%20op.%2079%20(280-329)%20obraz%20510-512.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.139.b.%20Pauline-polka%20pour%20piano%2C%20Nr%2010%2C%20op.%2079%20(280-329)%20obraz%20513-515.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.140. Ernst&ndash;polka, Victor &ndash; Polka, op. 79
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">2,3</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.140.%20Ernst%E2%80%93polka%2C%20Victor%20%E2%80%93%20Polka%2C%20op.%2079%20(366-368)%20obraz%20320-322.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c3 c8">C.141. Bertha &ndash; polka, op. 79</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.142. La fid&egrave;le Jeanette, polka, No 8, op. 79
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">2,3</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.142.%20La%20fidele%20Jeanette%2C%20polka%20no%208%2C%20op.79%20(366-368)%20obraz%20573.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">(only title page)</span>
      </p>
      <p className="c0">
        <span className="c3 c8">
          C.143. Marche de l&rsquo;op&eacute;ra &ldquo;Mois&eacute;e&rdquo; par
          Rossini, op. 79
        </span>
        <span className="c1">
          transcrit pour le piano par Jean Ruckgaber
        </span>
        <span className="c1 c11 c60">2,3,4</span>
        <span className="c1 c10 c15">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.143.%20Marche%20de%20l%E2%80%99op%C3%A9ra%20%E2%80%9CMois%C3%A9e%E2%80%9D%20par%20Rossini%2C%20op.79%20(280-329)%20obraz%20264-269.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.144. Andante interrompue par une marche fun&egrave;bre pour piano,
          op. 80
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">1862 </span>
        <span className="c1 c11 c60">2,5</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.144.%20Andante%20par%20Marche%20Funebre%20op.80.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.145.</span>
        <span className="c1">&nbsp;</span>
        <span className="c8 c3">Marche Funebre </span>
        <span className="c1 c11 c60">8 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.145.%20Marche%20Funabre%20BJ.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.146. Souvenir des bords du Sereth. op. 81,</span>
        <span className="c1">&nbsp;Valse avec Coda, &agrave; 4 mains, ms. </span>
        <span className="c1 c11 c60">1,2,3,7</span>
        <span className="c1">&nbsp;</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.146.%20Souvenir%20des%20bords%20du%20Sereth.%20op.%2081(366-368)obraz247-264.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.147. Fuga ........, op. 82 </span>
        <span className="c1 c11 c60">2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.147.%20Fuga%20..%2C%20op.%2082%20(280-329)%20obraz020-022a.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.148. Esmeralde &ndash; mazurka &eacute;l&eacute;gante, Es-dur, op.
          83
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.149. &bdquo;Mimi&rdquo; valse brillante, pour piano, A-dur, op. 84
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.149.%20%E2%80%9EMimi%E2%80%9D%20valse%20brillante%2C%20op.%2084(280-329)%20obraz%20389-393.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.150. Au G&eacute;n&eacute;ral Garibaldi grande marche &agrave; 4
          mains, op. 84
        </span>
        <span className="c1">, piano, ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.150.%20Au%20G%C3%A9n%C3%A9ral%20Garibaldi%20grande%20marche%20%C3%A0%204%20mains%2C%20op.%2084%20(280-329)%20obraz%20033b-034.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.151. 3 Marches &agrave; 4 mains, op. 84</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.151.%203%20Marches%20%C3%A0%204%20mains%2C%20op.%2084%20(280-329)%20obraz%20286-292.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3 c14">
          C.152.1. Souvenir de bords de la Moldawa, suite des valses pour piano,
          op. 85
        </span>
        <span className="c1 c14">, ms. </span>
        <span className="c1 c11 c60">1,2,3,5 </span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.152.1.a.%20Souvenir%20de%20bords%20de%20la%20Moldawa%20suite%20des%20valses%20pour%20piano%2C%20op.%2085.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
        <span className="c1 c14">, </span>
        <span className="c1 c10 c14">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.152.1.b.%20Souvenir%20de%20bords%20de%20la%20Moldawa%20suit%20de%20valse%20pour%20piano%2C%20op.%2085%20(366-368)%20obraz%20230-236.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">3</span>
      </p>
      <p className="c0">
        <span className="c8 c3 c14">
          C.152.2. Souvenir des Bords de la Moldova, Suite de Valses, op. 85, a
          4 Mains
        </span>
        <span className="c1 c14">,</span>
        <span className="c1 c42">&nbsp;ms. </span>
        <span className="c1 c11 c60">2,3 </span>
        <span className="c1 c10 c15">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.152.2.%20Souvenir%20des%20Bords%20de%20la%20Moldova%2C%20op.%2085%20a%204%20mains%20(366-368)obraz237-246.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c27 c8 c23 c15 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.153. Le songe, Fantaisie dramatique pour piano Des-dur, op. 87
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.153.%20Le%20songe%2C%20op.%2087%20(366-368)%20obraz%20207-217.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.154a. 3 Lieder ohne Worte f&uuml;r Piano, op. 88,
        </span>
        <span className="c1 c15 c3">
          a) Dumka b) Unruhe c) Venezianisches Gondollied
        </span>
        <span className="c1 c15">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c30 c25">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.154.a.%203%20Lieder%20ohne%20Worte%20f%C3%BCr%20Piano%2C%20op.%2088%20(280-329)%20obraz%20242-248.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.154b. 3 Romance sans paroles pour piano</span>
        <span className="c8">, op. 88, </span>
        <span className="c1 c3 c14">
          1) Chor der Studenten 2) Chor der Nonnen und M&ouml;nche 3) Chor der
          Krieger,{' '}
        </span>
        <span className="c1 c14">ms. </span>
        <span className="c1 c11 c60">2,3,4</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.154.b.%203%20Romance%20sans%20paroles%20Op.88%20(366-368)%20obr.%20082-089%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.154c. 6 Lieder ohne Worte f&uuml;r Piano, op. 88
        </span>
        <span className="c1 c11 c60">2,3</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.154.c.%206%20Lieder%20ohne%20Worte%20f%C3%BCr%20Piano%2C%20op.%2088%20(280-329)%20obraz%20249-263.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">a) Dumka</span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">b) Unruhe</span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">c) Venezianisches Gondel Lied</span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">d) Gesang der Krieger</span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">e) M&ouml;nch und Nonnen Chor</span>
      </p>
      <p className="c0">
        <span className="c1 c3">f) Schwedisches Burschenlied</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.155. Sonate pour piano, Es-dur, op. 89</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c13">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.155.a.%20(330-359)%20obraz%20242-251.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.155.b.%20Sonate%20pour%20le%20piano%2C%20op.%2089%20(330-359)%20obr.%20252-262.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">(see also H.33)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.156. Nocturne Es-Dur, op. 90 </span>
        <span className="c1 c11 c60">3,4,7</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.156.b.%20Nocturne%20Es-dur%20op.90%2C%20oczyszczony.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.157. 3-me Sonate du 2-de Quatuor transcript pour piano, op. 91
        </span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;1864,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.158. Chanson polonais populaire, op. 91</span>
        <span className="c1">,</span>
        <span className="c1 c3 c14">
          &nbsp;pour piano transcript par Jean Ruckgaber
        </span>
        <span className="c1 c11 c60">2,5</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.158.%20Chanson%20polonais%20populaire%2C%20op.%2091.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.159. Mazur, B-dur, op. 104</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.159.%20Mazur%2C%20B-dur%2C%20op.%20104%20(280-329)%20obraz%20306-309.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.160. Rondo fantastique pour piano, As-dur, op. 104
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.161. Pr&eacute;ludes dans toutes les tons de Major et Mi-mineur, op.
          104
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">1873</span>
        <span className="c1">, ms., </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.161.a.%20Preludes%20(366-368)%20obraz%20163-173.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">, </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.161.b.%20Pr%C3%A9ludes%20op.%20104%20(366-368)%20obraz%20174-186.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.162. Szenen aus den 4 Jahreszeiten, op. 123{' '}
        </span>
        <span className="c1 c11 c60">7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.163a. 1ste Lieblings-Mazur Ihrer Majest&auml;t der Kaiserin von
          Russland{' '}
        </span>
        <span className="c1 c11 c60">7</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.163b. 2te Lieblings-Mazur Ihrer Majest&auml;t der Kaiserin von
          Russland,{' '}
        </span>
        <span className="c1 c3">zu 4 H&auml;nden, 1840 </span>
        <span className="c1 c11 c60">7</span>
        <span className="c2 c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.163c. 3te Lieblings-Mazur Ihrer Majest&auml;t der Kaiserin von
          Russland,{' '}
        </span>
        <span className="c1 c3">zu 4 H&auml;nden, 1840 </span>
        <span className="c1 c11 c60">7 </span>
      </p>
      <p>
        <span className="c8 c3">C.164. Marche hongroise pour le piano</span>
        <span className="c1">, No 4, ms. </span>
        <span className="c1 c11 c60">2,5</span>
        <span className="c1">&nbsp;</span>
        <span className="c9 c1">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.164.%20Marche%20hongroise.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.165. 2 masures et une march </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c11 c45">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.165.%202%20masures%20et%20une%20marche%20(280-329)%20obr.%20293-295.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.166. Mazurka pour piano Marie Schier, 1869,{' '}
        </span>
        <span className="c1 c11 c60">8 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.166.%20Mazurka%2C%20BJ.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          C.167. M&eacute;lodies Valaques Roumania A-moll
        </span>
        <span className="c1">, 1863, ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.167.%20M%C3%A9lodies%20Valaques%20Roumania%20(280-329)%20obraz%20382-388.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c2 c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">C.168. Notezen Buch, </span>
        <span className="c1">ms. </span>
        <span className="c1 c11 c60">2,3,4</span>
        <span className="c1 c34">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/C.168.%20Notezen%20Buch%20(280-329)%20obraz435-461.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0 c21">
        <span className="c2 c1"></span>
      </p>

      <a className="section-ref" id="section-D" />
      <Typography variant="h3" gutterBottom>
        {' '}
        D. Music for the Violin and the Piano
      </Typography>
      <p className="c0">
        <span className="c8 c3">
          D. 1. Variations sur un th&egrave;me original, op. 32
        </span>
        <span className="c1 c11 c60">2,3,4</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/D.1.%20Variations%20sur%20un%20theme%20original%2C%20op.32%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">D. 2. Duo pour piano et violon, op. 34 </span>
        <span className="c1 c11 c60">2,9</span>
      </p>
      <p className="c0">
        <span className="c8 c3">D. 3. Duo pour piano et violon, op. 41 </span>
        <span className="c1 c11 c60">1,2,5</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/D.3.%20Divertissement%20%20Op.%2041.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
          ,{' '}
        </span>
        <span className="c1 c3">Divertissiment, violin, op.41 </span>
        <span className="c1 c11 c60">5</span>
        <span className="c1 c10">
          ,{' '}
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/D.3a%20Duo%20Op.41%20Divertissement%20Sacher%20Masoch.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c8 c10">,</span>
        <span className="c1 c10">&nbsp;</span>
        <span className="c1 c3">
          Duo Op.41, Divertissement, dedie Leopold de Sacher Masoch
        </span>
        <span className="c1 c11 c60">5</span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">D. 4. Duo, op. 44 </span>
        <span className="c1 c11 c60">1</span>
      </p>

      <a className="section-ref" id="section-E" />
      <Typography variant="h3" gutterBottom>
        {' '}
        E. Music for the Flute, the Czakan and the Piano
      </Typography>
      <p className="c0">
        <span className="c8 c3">E.1. Fantaisie pour Czakan et Piano, op.33 </span>
        <span className="c1 c11 c60">9 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/E.1%2C2%20Fantasie%20na%20czakan%20op.33%2C%20czyste%2C%20ca%C5%82o%C5%9B%C4%87.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          E. 2. Fantaisie brillante pour czakan avec fl&uuml;te douce et piano,
          op. 33
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,9</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/E.2.%20Fantaisie%20brillante%20pour%20Czakan%20ou%20Flute%20douce%20et%20Piano%20Forte%20op.33%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          E. 3. Introduction et variations pour czakan ou fl&uuml;te et piano
          op. 34
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">comp. 1833,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c6">&nbsp;</span>
        <span className="c19 c1 c23 c3">&nbsp;</span>
        <span className="c1 c9">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/E.3.a%2Cb.%20Introduction%20op%2034%20F366-368%20obr.560-571%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
       </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          E. 4. Duo pour fl&uuml;te et piano sur un th&egrave;me de
          &bdquo;Rigoletto&rdquo; et le grand air d&rsquo;&bdquo;Ernani&rdquo;,
          op. 92,
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">par Verdi</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2</span>
        <span className="c2 c1">&nbsp;</span>
      </p>
      <p className="c0 c21">
        <span className="c2 c1"></span>
      </p>

      <a className="section-ref" id="section-F" />
      <Typography variant="h3" gutterBottom>
        F. Music for the Clarinet and the Piano{' '}
      </Typography>
      <p className="c0">
        <span className="c8 c3">
          F. 1. Duo pour piano et clarinette in B, op. 18
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          F. 2. Sonate pour le Piano et Clarinette en Si -bemol,{' '}
        </span>
        <span className="c1 c3">op. 22</span>
        <span className="c1">, </span>
        <span className="c1 c3">par Luis Beethoven,</span>
        <span className="c6 c3">&nbsp;</span>
        <span className="c1 c3">transcript par J. Ruckgaber</span>
        <span className="c1">, </span>
        <span className="c8">op. 97,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/F.2.%20Sonate%20pour%20piano%20et%20clarinette%2C%20op.97.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c6">&nbsp;</span>
        <span className="c2 c1">(missing pages 22-23 of the manuscript)</span>
      </p>
      <p className="c0 c21">
        <span className="c2 c1"></span>
      </p>

      <a className="section-ref" id="section-G" />
      <Typography variant="h3" gutterBottom>
        G. Music for the Choir{' '}
      </Typography>
      <p className="c0">
        <span className="c8 c3">
          G. 1. Hoch- und Lustgesang-Cantate Es-dur, op. 57, (Dr. Rappaport){' '}
        </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/G.1.%20Cantate%20op.%2057%28366-368%29%20obraz%20543-554.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c13">&nbsp;</span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">G. 2. Mit Gott furs Vaterland, op. 63</span>
        <span className="c1">, </span>
        <span className="c1 c3 c14">
          gedicht v. F.t.Schwarz Musik v. Jan Ruckgaber, 1859{' '}
        </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10 c14">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/G.2%20oryg.%2C%20Mit%20Gott%20furs%20Vaterland%2C%20Op.63%20%28280-329%29%20obr.%20394-418.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">G. 3. Trauerlied, op. 67</span>
        <span className="c1">
          &nbsp;&ndash; Gesangbuch f&uuml;r Chor, Componiert von Johann
          Ruckgaber, 1859{' '}
        </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c8 c10">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/G.3.%20Trauerlied%20Op.67%20IM%20F366-368%20obr.%20283-285%2Cczysta.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          G. 4. Volks &ndash; Hymne &bdquo;Gott erhalte, Gott
          besch&uuml;tze&ldquo; As-dur, op. 70
        </span>
        <span className="c1">, </span>
        <span className="c1 c3 c14">f&uuml;r M&auml;nnerchor </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1 c10 c14">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/G.4.%20Volks%20%E2%80%93%20Hymne%20op.70%20%28330-359%29%20obraz%20323-324.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0 c21">
        <span className="c5 c3"></span>
      </p>

      <a className="section-ref" id="section-H" />
      <Typography variant="h3" gutterBottom>
        H. Music for the Voice and the Piano{' '}
      </Typography>
      <p className="c0">
        <span className="c8 c3">H. 1. Chanson chant&eacute;e, op. 1</span>
        <span className="c1">, </span>
        <span className="c1 c3">par Madame Becker </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.1.%20Chanson%20chantie%20op.%201%20%28280-329%29%20obr.297%2C%20czysta.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c34">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 2. Favorit-L&auml;ndler, op. 1</span>
        <span className="c1">,</span>
        <span className="c1 c3">
          &nbsp;voice, piano, von Johann Ruckgaber, 1822{' '}
        </span>
        <span className="c1 c11 c60">6,7</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.2.%20Landler.%20Op%201%20BN.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 3. 3 Lieder op. 2, 3, 4 </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.3.%203%20Lieder%20IM%20F280-329%20obr.232-241%2Cczysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">1) Abgelebte Tage</span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">2) Herzenblick</span>
      </p>
      <p className="c0">
        <span className="c1 c3">3) Der Trompeter, ms.</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 4. 2 Lieder </span>
        <span className="c1 c11 c60">1,2,3,4</span>
      </p>
      <p className="c0">
        <span className="c1">1) </span>
        <span className="c19 c1 c3 c23">Mein Herz, op. 5, fur Alt mit Piano</span>
      </p>
      <p className="c0">
        <span className="c1 c3">
          2) R. 1812, op. 6, v. Ludwig Relstab aus dem historischen Romance{' '}
        </span>
        <span className="c1">&ndash; </span>
        <span className="c1 c3">voice and piano</span>
        <span className="c1 c25 c3">&nbsp;</span>
        <span className="c16 c1 c25">&ndash; ms.</span>
      </p>
      <p className="c0">
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.4.a.%202%20Lieder%20fur%20Alt%20mit%20Piano%2C%20op.5%2C6%20(366-368)%20obr.%20580-582%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c3">2 Lieder</span>
        <span className="c1 c36 c3">,</span>
        <span className="c1 c10">&nbsp;</span>
        <span className="c1 c3">fur Alt mit Piano</span>
      </p>
      <p className="c0">
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.4.b.%20Mein%20Herz%20fur%20mezzo%20sopran%20280-329%20obr.327-328%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Mein Herz fur mezzo sopran mit piano</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H. 5. 3 Gedichte von Theodor Bakody, H-dur, As-dur, op. 9
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.6.%20Der%20Schifferknabe%2C%20Op.10%20(330-359)%20obr.%20271-278%2C%20%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 6. Der Schifferknabe, op. 10 </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.6.%20Der%20Schifferknabe%2C%20Op.10%20(330-359)%20obr.%20271-278%2C%20%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 7. Der Augenblick, op. 11</span>
        <span className="c1">, </span>
        <span className="c1 c3">
          dicht. von Alois Sebera, C-dur, Gesang u. Piano
        </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 8. </span>
        <span className="c8 c3 c14">
          3 Krakowiakes pour 2 voix avec l&rsquo;accompagnement du piano, op.
          12,
        </span>
        <span className="c8 c22 c11">&nbsp;</span>
        <span className="c8 c3 c14">Resignation Lied</span>
        <span className="c1 c14">,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.8.%203%20Krakowiakes%20op.12%2C%20Resignation%20Lied%20%28330-359%29%20obr.014-020%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 9. Liebes Zweifel, op. 13 </span>
        <span className="c1 c11 c60">2,3</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.9.a%20Liebes%20Zweifel%20op.13%20(366-368)%20obr.577-579%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c13">&nbsp;</span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H. 10. Mazure tir&eacute;e de la chanson, &bdquo;La
          Bajad&egrave;re&rdquo;, op. 18
        </span>
        <span className="c1 c11 c60">2</span>
        <span className="c1">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H. 11. 1) La M&eacute;lancholica, 3-&egrave;me nocturno, B-moll, op.
          19,
        </span>
        <span className="c1 c3">&nbsp;piano avec des paroles </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.11.1.%20La%20Melancholica%20op.19%20(280-329)%20obraz329-333.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2) M&ouml;ge
          mich ein zweites mal nie bereiten ein solches Schicksal,
        </span>
        <span className="c1 c3">&nbsp;1861</span>
        <span className="c1">, ms.</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 12. Du bist ein Engel, op. 21</span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;ged.v Alois Sebera, Gesang u. Piano </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H. 13. Romance pour le chant et piano, As-dur, op. 22, 23
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c19 c1 c23 c3">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.13.a.%20Romance%2C%20Chant%20%E2%80%93%20Piano%2C%20op.22%20(366-368)%20obr.105-107%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c3">Romance, Chant et Piano</span>
        <span className="c1">, </span>
        <span className="c1 c3">op.22</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">
          a M. le chevalier Anibal Ghergheril, manuscript
        </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.13.b.%20Romance%20pour%20le%20chant%20et%20piano%20Op.23%20(366-368)%20obr.090-092%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c3">
          Romance, Chant et Piano, op 23 a M. le chevalier Anibal Ghergheril,
          manuscript
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 14. Das Fischleinlied, G-dur, op. 24,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">sopran oder tenor, 1861 </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.14.a.%20Das%20Fischleinlied%20op.24%20(280-329)%20obr.%20001-003%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 15. Es schicht sich nicht, op. 26</span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;Gedicht von F. Elholtz,voice and piano</span>
        <span className="c1">., ms. </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.15.%20Es%20schicht%20sich%20nicht%20op.26%20(330-359)%20obr.%20052b-053%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 16. Wanda, op. 1</span>
        <span className="c1 c3">, Gedicht von Elholtz,</span>
        <span className="c1">&nbsp;voice and piano, ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.16.%20Wanda%20op.1%20(366-368)%20obr.%20325-328%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 17. </span>
        <span className="c8 c3 c14">Do mogi&#322;, op. 55</span>
        <span className="c1 c14">,</span>
        <span className="c1 c3 c14">
          &nbsp;lyrics by Wincenty Pol with German translation by Alojzy Sebera,
          voice and piano
        </span>
        <span className="c1 c11 c15 c3">2,5</span>
        <span className="c1 c15 c3">&nbsp;</span>
        <span className="c1 c10 c15">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.17.%20Do%20Mogi%C5%82%2C%20op.55.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
          7.{' '}
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H. 18. Diff&eacute;rentes compositions pour le chant, op. 61, 62, 63,
          64, 65, &agrave; 66, 67, 68, 69
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">
          compos&eacute;es dans l&rsquo;ann&eacute;e 1857, 1958 et 1859
        </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">
          1) 2 Volkshymne, for 4 male voices and a piano.
        </span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">
          2) Der Fr&uuml;hling, romance, voice and piano, manuscript from the
          year 1857
        </span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">
          3) Altes und Neues aue der Verse des wandelten, 1858
        </span>
      </p>
      <p className="c0">
        <span className="c1 c3">
          4) Gedicht von Moritz Saphier &ndash; for voice
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H. 19. a. Das Vater Unser samt Ave Maria, No 1, op. 65
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1 c3 c12">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.19.a.%20Das%20Pater%20unser%E2%80%93samt%E2%80%93Ave%20Maria%20op.65%20(366-368)%20obr.%20305-307czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H. 19. b. Das Vater Unser nebst Ave Maria, No 2, op. 65
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.19.b.%20(366-368)%20obr.308-310.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H. 20. Altes und Neues &ndash; Gesang u. Piano, op. 66
        </span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 21. Kennst das Meer, op. 68</span>
        <span className="c1">,</span>
        <span className="c1 c3">
          &nbsp;fur Bariton mit Forte Piano Begleitung, Johan Ruckgaber
        </span>
        <span className="c1 c11 c60">3,5</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.21.a.%20Kennst%20das%20Meer%20Op.68%2C%20Sopran%20%28366-368%29%20obr.329-332%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c25">&nbsp;</span>
        <span className="c1">,</span>
        <span className="c1 c25">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.21.b.%20Kennst%20das%20Meer%20Op.68%2C%20Bariton%2Cczysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c11 c60">5</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 22. Gendarmenlied, D-Dur, op. 69</span>
        <span className="c1">, </span>
        <span className="c1 c3">Bass mit Piano </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.22.a.%20Gendarmenlied%20op.69%20%28280-329%29%20obr.051-053.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 23. 3 M&eacute;lodies hongroises, op. 69 </span>
        <span className="c1 c11 c60">1,2,3,4</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.23.a%2Cb.%20oryg.%203%20Melodies%20hongroises%2C%20Op.69%20(280-329)%20obr.%20367-381.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c5 c3">&nbsp;,</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.23.%20Teksty%20(280-329)%20obraz%20%20373-%20375.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;(texts)</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H. 24. Verlassene Lied und Piano No 30, op. 70
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">M. Lanne Cassina gewidmet, </span>
        <span className="c1">ms. </span>
        <span className="c1 c11 c60">2,3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.24.%20Verlassene%20Lied%20und%20Piano%20No%2030%2C%20op.%2070%20%28366-368%29%20obraz%20312.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c16 c6">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 25. 3 pies&#769;ni: </span>
        <span className="c1 c11 c60">2,3,5</span>
      </p>
      <p className="c0">
        <span className="c5 c3">1) &nbsp;Hans und Grete, op. 91,</span>
      </p>
      <p className="c0">
        <span className="c8 c3">2) Der Himmelschl&uuml;ssel, op. 91</span>
        <span className="c2 c1">,</span>
      </p>
      <p className="c0">
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.25.a.%20Hans%20und%20Grete%2C%20%20Op.%2091%20(366-368)%20obr.%20538-542%2C%20rkp%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1 c3">2 pies&#769;ni: 1) Hans und Grete,</span>
        <span className="c6">&nbsp;</span>
        <span className="c1 c3">
          2) Mein Hertz, 3) Der Himmelschl&uuml;ssel, ms.
        </span>
        <span className="c1 c11 c60">2,3</span>
      </p>
      <p className="c0">
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.25.b.%20Die%20Himmelsschlussel%20Op.%2091%2C%20druk%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c3">Der Himmelschl&uuml;ssel, op. 91</span>
        <span className="c8 c3">, </span>
        <span className="c1 c3">print, </span>
        <span className="c1 c11 c60">5</span>
        <span className="c1 c10">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 26. Aus Immer Munchkausen, op. 92</span>
        <span className="c1">,</span>
        <span className="c1 c3">&nbsp;voice and piano</span>
        <span className="c1">, ms. </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 27. 3 Lieder, op. 94 </span>
        <span className="c1 c11 c60">1,2,3</span>
        <span className="c1">, </span>
        <span className="c1 c3">voice and piano,</span>
        <span className="c1">&nbsp;</span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.27.%203%20Lieder%20op.%2094%20und%20Der%20Fr%C3%BChling%20(366-368)%20obr.098-104%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">1) &nbsp;Rezygnacja</span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">2) Hans und Grete</span>
      </p>
      <p className="c0">
        <span className="c19 c1 c23 c3">3) Mein Herz, ms. </span>
      </p>
      <p className="c0">
        <span className="c1 c3">und 4) Der Fr&uuml;hling</span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H. 28. 1) Das erste Weib aus Engels Philosophie f&uuml;r die Welt, op.
          97
        </span>
        <span className="c1">, </span>
        <span className="c1 c3">Gesang&ndash;Piano </span>
        <span className="c1 c11 c60">1,2 </span>
        <span className="c1">&nbsp; </span>
        <span className="c8 c3">&nbsp; 2) Sachsisches Kriegs, </span>
        <span className="c1 c3">lied, 1866,</span>
        <span className="c1">&nbsp;ms. </span>
        <span className="c1 c11 c60">1,2</span>
        <span className="c1">&nbsp;</span>
        <span className="c5 c3">&nbsp;</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 29. Wir wollen Deutsche sein</span>
        <span className="c1">, </span>
        <span className="c1 c3">op. 20, f&uuml;r Chor 2 Tenor nur 2 Basse </span>
        <span className="c1 c11 c60">2,3 </span>
        <span className="c9 c1">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.29.%20Wir%20wollen%20Deutsche%20sein%2C%20op.%2020%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">H.30 2 Die Thr&auml;ne</span>
        <span className="c1">, op. 99 </span>
        <span className="c1 c11 c60">3 </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.30.%20%28366-368%29%20obraz%20339-340%2Coryg.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3 c34">&nbsp;</span>
        <span className="c8 c3">H. 31. 2 Chansons russes </span>
        <span className="c1 c11 c60">2</span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 32. Tenor solo zur Cantate, ms. </span>
        <span className="c1 c11 c60">2,3</span>
        <span className="c1 c10">
          &nbsp;
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.32.%20Tenor%20solo%20zur%20Cantate%20(366-368)%20obr.280-282%2C%20czysty.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">H. 33. Sonate pour le piano</span>
        <span className="c1 c3">
          &nbsp;par Jean Ruckgaber transcript du 2 de Bas tenor
        </span>
        <span className="c1 c11 c60">3</span>
        <span className="c1 c3">&nbsp; </span>
        <span className="c1 c10">
          <a
            href="https://storage.googleapis.com/geek-hosting.appspot.com/ruckgaber/compositions/H.33.%20Sonate%20pour%20le%20Piano%20%28330-359%29%20obr.225-241%2C%20czysta.pdf"
            target="_blank"
          >
            <PictureAsPdfIcon fontSize="small" style={{ color: red[500] }} />
          </a>
          /C.154{' '}
        </span>
      </p>
      <p className="c0">
        <span className="c8 c3">
          H.34. Dumka &bdquo;Bili zime&#808; ca&#322;a&#808;&rdquo;
        </span>
        <span className="c1">&nbsp;&ndash;</span>
        <span className="c1 c3">&nbsp;voice and piano</span>
        <span className="c1">&nbsp;&ndash; manuscript </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
      <p className="c0">
        <span className="c2 c1">&nbsp;</span>
      </p>

      <a className="section-ref" id="section-I" />
      <Typography variant="h3" gutterBottom>
        I. Music for the Guitar and the Piano{' '}
      </Typography>
      <p className="c0">
        <span className="c8 c3">I.1. Duo A-dur pour violon et guitarre</span>
        <span className="c1">, </span>
        <span className="c1 c3">par </span>
        <span className="c1 c3 c47">Joseph Mayseder,</span>
        <span className="c1 c15 c3 c47">&nbsp;</span>
        <span className="c1 c3">
          op.12, arrangee pour piano et guitarre, a son ami Charles Zehn.
          Dopisek: &ldquo;executee dans les socrees musicales de
          M.Kwiatkowski&rdquo; &ndash; manuscript{' '}
        </span>
        <span className="c1 c11 c60">1,2</span>
      </p>
    </div>
  )
}

export default CompositionsMarkup
